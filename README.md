...
- [x] **배당 성장 지표 추가**: 종목별 5년 배당성장률(CAGR) 및 연속 배당 지급 연수(배당귀족/배당킹) 뱃지 표시
...
- [x] **누적 시세차익 vs 배당 재투자 복리 차트**: 순수 시세 평가손익과 배당금 발생 시점마다 전액 재투자했을 때의 복리 수익률 분리 계산 (100% 재투자 기준)
- [x] **차트 안정성 & UX 최적화**: 미확정 종가(`null`/`0`) 필터링(-100% 급락 방지) 및 툴팁 이탈 방지(`confine: true`)
- [x] **월별 배당 캘린더 (달력 뷰)**: 1월~12월 달력 형태로 각 종목별 배당락일 및 배당지급 예정일 시각화
- [ ] **원클릭 포트폴리오 리밸런싱 가이드**: 추가 투자금 투입 시 목표 비중 달성을 위한 종목별 추가 매수 수량 가이드 계산기

### 5. 포트폴리오 저장, 공유 & 연동
- [x] **URL 파라미터 상태 영속화**: 입력 상태가 URL에 base64로 인코딩되어 링크 공유 및 브라우저 뒤로가기/앞으로가기 완벽 지원
- [ ] **Google/Apple 캘린더 실시간 배당 일정 구독 연동 (Webcal / iCal)**: URL 구독을 통해 스마트폰 및 PC 캘린더 앱에 배당 입금 일정을 자동 동기화
...
- [ ] **회원제 & 개인화 자산 관리 (장기 로드맵)**: 소셜 로그인 기반 클라우드 백업, 목표 마일스톤 달성률 추적 및 포트폴리오 공유 커뮤니티
